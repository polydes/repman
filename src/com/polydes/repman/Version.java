...
public class Version implements Comparable<Version>
{
	 ...
	 *            Micro component of the version identifier.
	 * @param qualifier
	 *            Qualifier component of the version identifier. If {@code null}
	 *            is specified, then the qualifier will be set to the empty
	 *            string.
	 * @throws IllegalArgumentException
	...
	public Version(String version)
	{
		...
		try
		{
			StringTokenizer st = new StringTokenizer(version, SEPARATOR + QUALIFIER_SEPARATOR, true);
			maj = parseInt(st.nextToken(), version);

			if (st.hasMoreTokens())
			{ // minor
				assertSame(st.nextToken(), SEPARATOR, version); // consume delimiter
				...
				{ // micro
					...
					if (st.hasMoreTokens())
					{ // qualifier separator
						assertSame(st.nextToken(), QUALIFIER_SEPARATOR, version); // consume delimiter
	...
	private static void assertSame(String a, String b, String version)
	{
		if(!a.equals(b))
			throw new IllegalArgumentException("invalid version \"" + version + "\": invalid format");
	}
	 ...
	 *            Numeric component
	 * @param version
	 *            Complete version string for exception message, if any
	 ...
	 * @param version
	 *            String representation of the version identifier. Leading and
	 *            trailing whitespace will be ignored. Must not be {@code null}.
	 * @return A {@code Version} object representing the version identifier. If
	...
	String toString0()
	{
		...
		if (q > 0)
		{
			result.append(QUALIFIER_SEPARATOR);
